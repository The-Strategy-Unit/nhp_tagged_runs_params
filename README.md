...
* a list object where each element contains the parameters ('params') for each New Hospital Programme (NHP) model scenario that's been labelled with a 'run stage' (you can [see a table of these 'tagged runs'](https://connect.strategyunitwm.nhs.uk/nhp/tagged_runs) if you have permission)
* a data.frame object containing metadata associated with these runs
...
* Azure Table Storage, where we have a lookup that lists scenarios with a run stage
...
The deployed report runs on Posit Connect on schedule, so any changes to the underlying data will be integrated on the next rendering.
...
Internal develoeprs can ask a member of the Data Science team for the values required by each variable.
